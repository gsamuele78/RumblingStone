...
| `validate_corredo.py` | **Il corredo della serata** (norma in `rumblingstone-automation`): da un `*.corredo.json` controlla che ci siano booklet del DM, volume dei fogli, echi di ogni PG, carte e handout, che ogni foglio ✉ del booklet stia nel volume dei fogli, che ogni immagine dei prompt esista e abbia la sua riga nel confronto immagine-scheda, e che nessun box superi le 12 righe. Con `--stampa` compila ogni volume e misura il PDF (sovrapposizioni, testo a meno di 30 pt dal bordo; PyMuPDF facoltativo). `dm.py corredo` lo chiama prima e dopo i volumi | `[corredo…]` · `--stampa` · `--pdf` · `--rigoroso` · `--json` | i `*.corredo.json` del repo e ciò che citano; **typst** per `--stampa`, **PyMuPDF** facoltativo (se manca, la misura del PDF si dichiara saltata) | exit 0/1/2 + elenco dei difetti (gira in CI) |
