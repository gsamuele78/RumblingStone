# Le sei famiglie di caso

Il congegno è **una** delle risposte possibili, non la struttura. Tutto
l'impianto (nodi a tre strati, sei porte, ricomposizione, falsi indizi,
vicolo cieco, Acume e Perizia) è identico in tutte e sei le famiglie: cambia
**cosa** si nasconde, e quindi che tipo di dettagli si posano.

Si sceglie la famiglia **prima** di scrivere i nodi. È la decisione che
determina la firma degli indizi.

> **Si possono mescolare due famiglie. Mai tre.** Un omicidio dentro una
> cospirazione funziona; un omicidio dentro una cospirazione con un
> impostore e un congegno è un caso che nessuno ricostruirà mai — nemmeno
> tu, tre sessioni dopo.

---

## 1. Chi è stato — omicidio, furto, sabotaggio

**La domanda**: chi l'ha fatto, e perché proprio adesso.

**La firma negli indizi**: lo scarto fra **la storia raccontata e la roba
fisica**. Qualcuno dice una cosa e la stanza ne dice un'altra. Le sei porte
brillano qui più che altrove: FOR dice quanti erano, DES da che parte è
partito il colpo, COS quanto ci ha messo a morire.

**La chiave tipica**: il dettaglio che **solo il colpevole poteva lasciare o
sapere**, e che finora sembrava di tutti.

**L'errore da evitare**: il colpevole mai comparso prima. Un nome nuovo alla
rivelazione non è una sorpresa, è un imbroglio. **Il colpevole va conosciuto,
e possibilmente ci hanno parlato.**

**Come finisce**: quasi mai in un arresto. Finisce nella **scelta di cosa
farne**, e quella scelta scrive un'eco.

## 2. L'impostore — qualcuno non è chi dice

**La domanda**: quale di questi non è sé stesso. (E, più tardi: da quanto.)

**La firma negli indizi**: **errori di competenza**, non di racconto. Sa una
cosa che non dovrebbe sapere, e non sa una cosa che chiunque al suo posto
saprebbe. Non ricorda un'abitudine. Scrive con la mano giusta ma tiene il
boccale con l'altra.

**La chiave tipica**: la cosa che **la persona vera non avrebbe mai potuto
dimenticare**: un lutto, un debito, una cicatrice, il nome di un cane.

**L'errore da evitare**: renderlo risolvibile con un tiro di Intuizione. La
prova sociale dà **una sensazione**, mai il nome; il nome viene
dall'accumulo. E il secondo errore: un solo impostore ovvio. Se il tavolo ha
un solo sospettato, non sta indagando, sta aspettando conferma.

**Come finisce**: **una scena sociale, non uno scontro.** Lo smascheramento
davanti a chi conta vale dieci volte l'agguato in un vicolo.

## 3. La cospirazione — molte persone, uno scopo

**La domanda**: chi sta dalla stessa parte, e da quando.

**La firma negli indizi**: **coincidenze di tempo e di denaro**. Gente che non
dovrebbe conoscersi e si conosce. Tre decisioni prese lo stesso giorno da tre
uffici diversi. Un prezzo che si muove prima della notizia.

**La chiave tipica**: **il nodo che collega**: un nome, un pagamento, una
stanza dove sono passati tutti.

**L'errore da evitare**: farla troppo grande per essere toccata. Vale il
vincolo del pilastro 5: **la conseguenza politica deve arrivare a misura di
PG**: una tassa, un sicario, un invito. Una cospirazione che riguarda solo i
regni è un fondale.

**Come finisce**: mai con tutti in catene. Si taglia **un pezzo**, e il resto
impara e si riorganizza, che è materia per l'arco dopo.

## 4. La sparizione — qualcuno o qualcosa non c'è più

**La domanda**: andato via, portato via, o nascosto?

**La firma negli indizi**: **l'assenza stessa**, letta con precisione. Cosa
manca dalla stanza e cosa no. **Cosa una persona non si è portata dietro**,
è la cosa che dice tutto: chi fugge prende i soldi, chi viene preso non prende
niente, chi vuole sparire lascia apposta quello che lo identifica.

**La chiave tipica**: se ha fatto i bagagli. Sul serio: è quasi sempre quella.

**L'errore da evitare**: che la ricerca diventi un itinerario. Tre villaggi e
un bosco non sono tre nodi: sono viaggio. I nodi stanno nelle **persone che
l'hanno vista**, non nei chilometri.

**Come finisce**: spesso chi si ritrova **non vuole tornare**. Quello è il
caso, non il ritrovamento.

## 5. L'accusa falsa — qualcuno (magari un PG) è incolpato

**La domanda**: a chi conviene che sia creduta?

**La firma negli indizi**: **le prove sono troppo buone.** Tutto torna, tutto
è a portata di mano, tutto è arrivato al momento giusto. La realtà non è mai
così ordinata, e questo è l'unico caso in cui *«tutto è significativo»*
(`italiano-nativo.md` §9) è un indizio invece che un tic.

**La chiave tipica**: **l'unico difetto della messinscena**: la cosa che chi
ha costruito le prove non poteva sapere.

**L'errore da evitare**: mettere il gruppo in cella e togliergli il gioco. Se
un PG è accusato, deve poter **agire**: in fuga, in libertà vigilata, con un
termine che scade. Il tempo è la pressione, non le sbarre.

**Come finisce**: scagionarsi **non ripara il danno**. Chi ha creduto
all'accusa continua a crederci un po', e quello resta addosso per archi.

## 6. Il congegno — cosa si sta costruendo

**La domanda**: cosa fa la macchina, e cosa succede quando è finita.

Famiglia con un file suo: `congegno-e-enigmi.md`, con le tre sottofamiglie
(arcano, aberrante, mistico), la regia del prestigiatore, l'innesco in piena
vista, l'agenda dei villain, il congegno come terzo attore in combattimento e
il dosaggio.

⚠️ **È la famiglia più costosa in stupore**: si usa **una volta per arco**, e
le altre cinque coprono tutto il resto del tempo. Un DM che scrive solo casi
di questa famiglia consuma in tre sessioni una risorsa che si ricarica lenta.

---

## Lo scheletro, generico

Lo stesso di `SKILL.md` §4, con la prima riga che cambia nome secondo la
famiglia:

```
IL SEGRETO       la verità intera, che il DM sa dal primo minuto.
                 (colpevole · vera identità · chi è alleato di chi ·
                  dov'è finito · chi ha costruito le prove · la macchina)
LA CHIAVE        l'informazione che ricodifica tutto in una volta, a ~2/3.
LE SCHEGGE       6-9 nodi, ognuno con la sua INNOCENZA.
CHI CI GUADAGNA  chi vuole che la verità non si sappia, e cosa fa nel
                 frattempo (mondo vivo: agisce, non aspetta).
L'INNESCO        il dettaglio in piena vista che scatta se nessuno lo tocca.
IL COSTO         cosa peggiora mentre i PG capiscono.
LA PORTA CHIUSA  la cosa che non capiranno mai del tutto.
```

**`L'INNESCO` è opzionale fuori dalla famiglia 6**: un caso può non averne.
Tutte le altre righe sono obbligatorie in tutte e sei.

## Scegliere in trenta secondi

| Se vuoi che il tavolo… | Famiglia |
|---|---|
| guardi le cose e le persone e trovi lo scarto | 1 — chi è stato |
| smetta di fidarsi di un PNG che gli piaceva | 2 — l'impostore |
| capisca che la Valle è più marcia di quanto pensava | 3 — la cospirazione |
| provi urgenza pura, con un orologio | 4 — la sparizione |
| giochi sulla difensiva, con la reputazione in gioco | 5 — l'accusa falsa |
| resti a bocca aperta davanti a una cosa | 6 — il congegno |
