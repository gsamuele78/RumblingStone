...
| [PIANO-LEVEL-DESIGN-E-INQUADRATURA](PIANO-LEVEL-DESIGN-E-INQUADRATURA-SCENICA.md) | Infrastruttura + contenuto: legenda **funzionale** fonte unica ([ADR-0014](adr/ADR-0014-legenda-funzionale-fonte-unica.md), = lotto E1 del PIANO-EDITOR) · **substrato di ingegneria** (pyproject/pacchettizzazione, dipendenze a livelli [ADR-0015](adr/ADR-0015-dipendenze-a-livelli-e-pacchettizzazione.md), ruff/pytest, confine dominio/presentazione) · linter di level design `lint_map_design.py` (M1-M9, advisory) · elevazione/zone/`design_intent` nel contratto · scheda-mappa · Modalità 4 blockout 3D→depth · ordine percettivo nel read-aloud | 🔵 **pianificato** (2026-07-26) | 0% | **0.1-0.4 (substrato, rev. 2)** · A1-A3 (dati) · B1-B5 (linter) · C1-C3 (mappe) · D1-D4 (inquadratura/prosa) — sequenza consigliata in §4 del piano | ADR-0015: decisione DM su dipendenze a livelli + pacchettizzazione · A1: decisione DM sulle 4 classificazioni divergenti (`⛰ 🏛 🗼 🗿`) · B5: corpus di calibrazione libero (mai RHoD, ADR-0005) · C2/D2: prova al tavolo · D3: GPU + Blender sulla macchina DM |
| [PIANO-PRODOTTO-TOOLKIT-VENDIBILE](PIANO-PRODOTTO-TOOLKIT-VENDIBILE.md) | **Prodotto**: da repo di campagna a toolkit vendibile — separazione dei due prodotti e rilicenziamento ([ADR-0017](adr/ADR-0017-separazione-prodotto-e-rilicenziamento-toolkit.md)), legenda funzionale neutra a 62 simboli ([ADR-0014](adr/ADR-0014-legenda-funzionale-fonte-unica.md) + [spec](../docs/guides/LEGENDA-FUNZIONALE-SPEC.md)), **profili di regole 3.5 / PF1e / 5e** ([ADR-0016](adr/ADR-0016-profili-regole-multisistema.md)), confezionamento wheel + eseguibile autonomo ([ADR-0015](adr/ADR-0015-dipendenze-a-livelli-e-pacchettizzazione.md)), linter, map pack neutri, guida al metodo | 🔵 **pianificato** (2026-07-26) | 0% | P0.1-P0.4 (substrato) · P1.1-P1.3 (legenda + 3 sistemi) · P2.1-P2.4 (linter) · P3.1-P3.4 (wheel/exe/licenze) · P4.1-P4.2 (contenuto) · P5.1 (gate) — primo rilascio possibile dopo il passo 4 di 8 | ⚠️ **avvocato IP prima di qualunque vendita** (rilicenziamento, OGL 1.0a, CC BY 4.0, marchi) · PF1e `move_cost: 4` da confermare sul PRD · corpus di calibrazione con licenza documentata (mai RHoD, ADR-0005) |
...
- ⬜ _(A1+0.4 → A3+A2 → B1+B4 → C2 → D1+D4 → B2/B3/B5 → D3 → C3: sequenza e stime in §4 del piano)_
