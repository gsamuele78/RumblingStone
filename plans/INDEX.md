...
| [PIANO-REVISIONE-GLOBALE-2026-08](PIANO-REVISIONE-GLOBALE-2026-08.md) | Trasversale: audit su tre assi — **PRD/editoriale/pubblicazione**, **coerenza di flusso-storia-narrazione** (`[INFERRED]` inclusi), **ingegneria/ADR/testing/CI-CD** — con focus tavolo adulti | 🟡 **in esecuzione**: G0 (audit), **G1** (due tempi), **G2** (deriva doc↔realtà) **G2-bis** (`state.yaml` + ADR-0017) **G2-ter** (pipeline sessione→stato) e **G2-quater** (prodotto vs partita) chiusi il 2026-08-05 | ~41% (7 di 17 lotti) | G3-G4 subito (nessuna decisione DM) · G5-G6 su decisione DM · G7-G9 infrastruttura · G10-G11 gated | G5/G6: decisioni DM · G3: sorte dei 13 asset Palio · G10: prima sessione reale · G11: intervista di ricostruzione |
...
- ⚠️ _(**due domande al DM aperte da G1**, marcate `[INFERRED]` dove vivono e registrate in `state.md` §8: (1) il **−2 COS permanente di Thorik** è il prezzo della resurrezione non ancora giocata o è residuo della sua morte all'Arco 00? (2) il **Giorno di Marcia** è **19** come dice §2.1 o **~15** come implica l'orologio Hammerfist a 3g 16h? Non risolte d'ufficio: il Giorno di Marcia alimenta i numeri di §2.4 e la finestra quest di Arco 09)_
...
- ✅ _(G3 — chiuso 2026-08-06 **salvo la decisione A1**: gate `validate_links.py` bloccante su tutti i `.md`; 18 link rotti e 25 path assoluti trovati, di cui **due classi intere di falsi positivi** (deploy dei converters = path di *server*; documenti d'audit che citano il difetto). Riparati i 2 `cd /home/jfs/` del PLAYBOOK, i 2 link `file:///` della scheda Bracieri, l'immagine dell'Anello (`.webp` → `.png`) e un bug mio di G2-quater. **I 13 asset del Palio attendono la tua decisione**: marcati, con nota che dichiara il debito e un test che la presidia)_ <!-- validate-links: ignore -->
