...
| [PIANO-REVISIONE-GLOBALE-2026-08](PIANO-REVISIONE-GLOBALE-2026-08.md) | Trasversale: audit su tre assi — **PRD/editoriale/pubblicazione**, **coerenza di flusso-storia-narrazione** (`[INFERRED]` inclusi), **ingegneria/ADR/testing/CI-CD** — con focus tavolo adulti | 🟡 **in esecuzione**: G0 (audit), **G1** (due tempi), **G2** (deriva doc↔realtà) **G2-bis** (`state.yaml` + ADR-0017) **G2-ter** (pipeline sessione→stato) e **G2-quater** (prodotto vs partita) chiusi il 2026-08-05 | ~40% (6 di 15 lotti) | G3-G4 subito (nessuna decisione DM) · G5-G6 su decisione DM · G7-G9 infrastruttura · G10-G11 gated | G5/G6: decisioni DM · G3: sorte dei 13 asset Palio · G10: prima sessione reale · G11: intervista di ricostruzione |
...
- ⚠️ _(**decisione rimandata a G5**: branch-per-gruppo o `groups/<nome>/`? La seconda è strutturalmente migliore ma costa ~10 script group-aware. Dipende da chi è il destinatario del repo — seconda decisione che si incaglia sul PRD)_
- ⬜ _(G3-G4 — 17 link rotti + 4 path `/home/jfs/…` committati · inventario e ratchet dei 379 `[INFERRED]`)_
