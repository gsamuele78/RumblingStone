...
| [PIANO-INCANTATORI-MEMORABILI](PIANO-INCANTATORI-MEMORABILI-MANO-ROSSA-E-DROW.md) | Contenuto: incantatori nuovi per Mano Rossa (generale ogre magi elementalista + comprimari ogre sotterranei) e drow di Sonjak (illusionista-ombra, trickster arcano, assassina) — 6 schede + 2 dossier, tarati sulle difese reali del party (Mind Blank di Thorik, vulnerabilità al fuoco di Hella) | 🟢 **eseguito** (2026-08-02): L1-L5 completi, `validate_bestiario` verde | ~90% | L6 **gated su DM**: approvazione INFERRED→ACCEPTED, righe `state.md` §3/§4, token | conferma DM sui PNG proposti |
...
- ⬜ _(L6, gated su DM: approvare o scartare Ghaurush «Cenerevento» e Zin'thara Vel'Ryn; se approvati → flag `[ACCEPTED — DM-canon]`, clock e conoscenze in `campaign/state.md` §3/§4, token in `Bestiario/tokens/`)_
- ⬜ _(opzionale, decisione DM: scheda fomoriano da fonte privata `[Private source]` come ogre deforme del Sottosuolo — oggi non scritta per policy SRD)_
